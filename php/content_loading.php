...
<?php
// Pagination - 9 posts par page (3x3)
$posts_per_page = 9;
$page = isset($_GET['page']) ? max(1, (int) $_GET['page']) : 1;
$offset = ($page - 1) * $posts_per_page;

// Compter le total
$count_stmt = $mysqlConnection->prepare("
...
$stmt = $mysqlConnection->prepare("
    SELECT 
        ...
        COUNT(DISTINCT likes.id_utilisateur) 
    FROM contenus 
    JOIN utilisateurs ON utilisateurs.id = contenus.id_utilisateur 
    LEFT JOIN likes ON likes.id_contenu = contenus.id 
    GROUP BY contenus.id, contenus.description, utilisateurs.pseudo
...
for ($i = 0; $i < count($list_contenu); $i++) {
    ...
    $cols[$compteur][] = "<li class=\"post\">"
        ...
        . "</a>"
        . "<a href=\"page/partage.php?id=".$list_contenu[$i][1]."\" class=\"button-like-partage\">"
...
while ($cols != [[], [], []]) {
    ...
    // Vérifie que l'index existe avant de l'utiliser
    if ($compteur >= 3)
        break;

    if (isset($cols[$compteur]) && count($cols[$compteur]) > 0) {
